(function(){
  ...
  function eraLabel(era){
    ...
    return ERA_MAP[era.id] || era.era;
  }

  function packDisplayName(pack){
    if(lang()==='ja') return pack.name || pack.code || pack.apiId || '';
    return pack.code || (pack.apiId ? String(pack.apiId).toUpperCase() : '') || pack.name || '';
  }

  function packMetaText(pack){
    const code = String(pack.code || pack.apiId || '').trim();
    if(lang()==='ja') return `${code} · ${pack.cards || '?'}枚${pack.note ? ' · ' + pack.note : ''}`;
  ...
  async function showCardModalJPPatched(card, pack){
    ...
    const fallbackImg = deriveFallbackImage(fullCard, effectivePack) || deriveFallbackImage(card, effectivePack);
  ...
  function patchRenderDexSeries(){
    if(typeof window.renderDexSeries !== 'function' || window.__psDexSeriesV3Patched) return;
      ...
      SERIES_DATA.forEach((era) => {
        ...
        filtered.forEach((p) => {
          ...
          if(relatedCards.length > 0){
            ...
            cardsSection.innerHTML = `<div class="series-cards-title">${escSafe(tr('💰 高額レアカード','💰 Featured chase cards'))}</div>` +
              relatedCards.map((c) => {
    ...
    window.__psDexSeriesV3Patched = true;
  }

  function patchSetLang(){
    if(typeof window.setLang !== 'function' || window.__psDexLangV3Patched) return;
    ...
    window.__psDexLangV3Patched = true;
  }

  function patchShowCardModalJP(){
    window.showCardModalJP = function(card){ return showCardModalJPPatched(card, card?._pack || null); };
  }
  ...
  function boot(){
    ...
    const search = document.getElementById('dex-search');
    if(activeSeries && typeof renderDexSeries === 'function') renderDexSeries(search?.value || '');
  }
